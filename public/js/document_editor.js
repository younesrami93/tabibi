// MediDesign Pro v3.5 - Bootstrap Compatible
...
const D = document, W = window, L = localStorage, J = JSON, M = Math,
    ...
    sel = (i, skipRen) => {
        selId = i;
        Array.from(D.getElementsByClassName('doc-element')).forEach(e => e.classList.toggle('selected', e.id == selId));
    ...
    ren = () => {
        ...
        d.elements.forEach((e, i) => {
            ...
            if (e.type == 'text') b.ondblclick = () => edit(inr, e);
            c.append(b);
        });
        renLayers();
        updUI();
    ...
    renLayers = () => {
        // UPDATED: Used Bootstrap utility classes (d-flex, gap-2, text-danger) instead of Tailwind
        ...
        [...d.elements].reverse().forEach(e => {
            ...
            <div class="d-flex align-items-center gap-2 flex-grow-1">
                ...
                <input class="bg-transparent border-0 outline-none w-100 small text-light" value="${e.name || e.type}" onchange="renLay('${e.id}',this.value)">
            </div>
            <div class="d-flex align-items-center gap-2 opacity-50 hover-opacity-100">
                <button class="btn btn-sm p-0 text-light" onclick="mvLay('${e.id}',1)"><i class="fa-solid fa-chevron-up"></i></button>
                <button class="btn btn-sm p-0 text-light" onclick="mvLay('${e.id}',-1)"><i class="fa-solid fa-chevron-down"></i></button>
            </div>`;
    ...
    updUI = () => {
        // UPDATED: Swapped 'hidden'/'flex' for 'd-none'/'d-flex'
        ...
        // UPDATED: Button classes for Bootstrap theme
        ID('btnLock').className = `btn btn-sm flex-fill ${el.locked ? 'btn-danger text-light' : 'bg-theme-base border-theme text-light'}`;
    },

    // --- INTERACTIONS (Unchanged logic, just DOM manipulation) ---
    initDrg = (e, div) => {
    ...
    edit = (inr, e) => {
        if (e.locked) return; inr.contentEditable = true; inr.focus(); inr.classList.add('editing');
        const r = D.createRange(); r.selectNodeContents(inr); const s = W.getSelection(); s.removeAllRanges(); s.addRange(r);
        inr.onblur = () => { inr.contentEditable = false; inr.classList.remove('editing'); e.content = inr.innerText; SV(); ren(); }
    };
...
// UPDATED: Replaced Tailwind active classes with Bootstrap specific active/inactive states
W.switchTab = t => {
    ['props', 'layers'].map(k => {
        let b = ID('tab-' + k), p = ID('panel-' + k);
        if (k == t) {
            // Active Tab Style
            b.classList.add('border-primary', 'text-primary', 'bg-theme-base', 'border-bottom', 'border-2');
            b.classList.remove('text-muted', 'hover-bg-theme');
            p.classList.remove('d-none');
            p.classList.add('d-flex');
        } else {
            // Inactive Tab Style
            b.classList.remove('border-primary', 'text-primary', 'bg-theme-base', 'border-bottom', 'border-2');
