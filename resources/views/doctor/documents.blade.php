                    ...
                    <div class="card   hover-elevate transition">
                        <a href="{{ route('documents.editor', $doc->id) }}" class="text-decoration-none text-dark">
                            <div class="card-body pb-0 text-center p-4">
                                <div class="mb-3">
                                    <div class="avatar avatar-lg rounded-circle bg-light-primary text-primary mx-auto d-flex align-items-center justify-content-center"
                        ...
                        <div
                            class="card-footer bg-transparent border-top-0 d-flex justify-content-between align-items-center px-4 pt-0">
                            <small class="text-muted" style="font-size: 0.75rem;">
                                Edited {{ $doc->updated_at->diffForHumans() }}
                            </small>

                            <div class="dropdown">
                                <button class="btn btn-sm btn-icon btn-light rounded-circle" type="button"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
                                    ...
                                    <li>
                                        <form action="{{ route('documents.duplicate', $doc->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="dropdown-item">
                                                <i class="fa-regular fa-copy me-2 text-info"></i> Duplicate
                                            </button>
                                        </form>
                                    </li>
            ...
            <form action="{{ route('documents.store') }}" method="POST" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        {{-- UPDATED: Dropdown using Model Constants --}}
                        <select name="role" class="form-select">
                            @foreach(\App\Models\Document::getRoleLabels() as $value => $label)
                                <option value="{{ $value }}" {{ $value === 'general' ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form id="editDocForm" method="POST" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-uppercase text-muted">Role / Category</label>
                        {{-- UPDATED: Dropdown for Edit --}}
                        {{-- The JavaScript responsible for opening this modal will need to set .value on this ID --}}
