...
<div class="container-fluid p-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
        ...
        <div class="d-flex align-items-center bg-white border shadow-sm rounded-pill px-2 py-1" style="min-width: 280px;">
            <div class="p-2 text-muted"><i class="fa-regular fa-calendar"></i></div>
            <input type="text" id="reportRange" class="form-control border-0 bg-transparent shadow-none fw-bold text-dark text-center" style="font-size: 0.95rem; cursor: pointer;">
    ...
    <form action="{{ route('finance.index') }}" method="GET" id="financeFilterForm">
        <input type="hidden" name="date_from" id="inputDateFrom" value="{{ request('date_from', now()->startOfMonth()->format('Y-m-d')) }}">
        ...
        <div class="row g-3 mb-4">
                ...
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body p-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="fw-bold text-success mb-0">+{{ number_format($stats['income'], 2) }} <small class="fs-6 text-muted">DH</small></h3>
                        </div>
                ...
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body p-4 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="fw-bold text-danger mb-0">-{{ number_format($stats['expense'], 2) }} <small class="fs-6 text-muted">DH</small></h3>
                        </div>
            ...
            {{-- Balance --}}
            <div class="col-md-4">
                <div class="card bg-light-primary border-primary border-0 shadow-sm h-100">
                    <div class="card-body p-4 d-flex justify-content-between align-items-center">
                        <div>
                            <p class="text-uppercase small fw-bold text-muted-50 mb-1">Net Balance</p>
                            <h3 class="fw-bold text-black mb-0">{{ number_format($stats['balance'], 2) }} <small class="fs-6 text-muted-50">DH</small></h3>
                        </div>
      ...
      <button class="btn btn-primary fw-bold shadow-sm px-4 rounded-pill" data-bs-toggle="modal" data-bs-target="#addTransactionModal">
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-header bg-white border-bottom py-3">
                <div class="row g-2 align-items-center">
                    ...
                    {{-- Filters --}}
                    <div class="col-md-2">
                        <select name="type" class="form-select form-select-sm bg-light border-0 fw-medium" onchange="this.form.submit()">
                    ...
                    <div class="col-md-2">
                        <select name="method" class="form-select form-select-sm bg-light border-0 fw-medium" onchange="this.form.submit()">
                    ...
                    <div class="col-md-2 text-end">
                        <a href="{{ route('finance.index') }}" class="btn btn-light btn-sm text-muted border fw-bold w-100">
                        ...
                        <tr>
                            ...
                            <td class="border-bottom-0">
                                ...
                                @if($trx->billable_type === 'App\Models\Appointment')
                                    <span class="badge bg-primary bg-opacity-10 text-primary ms-1">Appt #{{ $trx->billable_id }}</span>
                                @endif
                            </td>
                            <td class="border-bottom-0">
                            ...
                            <td class="border-bottom-0">
                                <span class="text-capitalize small"><i class="fa-regular fa-credit-card me-1"></i> {{ $trx->payment_method }}</span>
                            </td>
                            <td class="text-end pe-4 border-bottom-0">
                                <span class="d-block fw-bold fs-6 {{ $trx->type === 'income' ? 'text-success' : 'text-danger' }}">
                                    {{ $trx->type === 'income' ? '+' : '-' }} {{ number_format($trx->amount, 2) }}
                                </span>
                            </td>
                            <td class="text-end border-bottom-0">
                                <form action="{{ route('finance.destroy', $trx->id) }}" method="POST" onsubmit="return confirm('Delete transaction?');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-sm text-muted hover-danger"><i class="fa-regular fa-trash-can"></i></button>
                                </form>
                            </td>
                        </tr>
                        @empty
